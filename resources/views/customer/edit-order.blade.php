                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('order.update', $order->order_id) }}" method="POST">
                            ...
                            <select name="service_id" required class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @foreach($services as $service)
                                    <option value="{{ $service->service_id }}"
                                            {{ $orderService && $orderService->service_id == $service->service_id ? 'selected' : '' }}>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="number" name="quantity" min="1" max="15" step="0.1"
                                   value="{{ $orderService ? $orderService->quantity : '' }}" required
                        ...
                        <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded">
                            <strong>Current Total:</strong> ${{ number_format($order->total_price, 2) }}
                        </div>
