<nav x-data="{ open: false }" class="bg-white border-b border-gray-100 sticky top-0 z-50">
    {{-- 1. ANTI-FLASH STYLE --}}
    <style>
        [x-cloak] { display: none !important; }
    </style>

    <div class="max-w-[90rem] mx-auto px-4 md:px-10">
        <div class="flex justify-between h-20"> 
            
            {{-- Brand Section --}}
            <div class="flex items-center">
                <a href="/" class="shrink-0 flex items-center group">
                    <div class="mr-3">
                        <svg width="50" height="42" viewBox="0 0 50 42" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                            <rect width="50" height="42" fill="url(#pattern0_54_93)"/>
                            <defs>
                                <pattern id="pattern0_54_93" patternContentUnits="objectBoundingBox" width="1" height="1">
                                    <use xlink:href="#image0_54_93" transform="matrix(0.0025 0 0 0.00297619 -0.00875 0)"/>
                                </pattern>
                                <image id="image0_54_93" width="407" height="336" preserveAspectRatio="none" xlink:href="data:image/png;base64,"/>
                            </defs>
                        </svg>
                    </div>
                    <div>
                        <div class="flex items-center group cursor-pointer">
                            <span class="text-2xl font-medium uppercase tracking-[0.1em] leading-none transition-all duration-300">
                                <span class="text-slate-700">Fresh</span><span class="text-[#7c4dff]">Press</span>
                            </span>
                        </div>
                        <span class="flex items-center gap-1.5 text-[10px] text-gray-500 uppercase tracking-widest font-bold mt-1.5">
                            @auth 
                                <span class="text-gray-900">{{ auth()->user()->getRoleNames()->first() }} PANEL</span> 
                                <span class="text-indigo-400 font-light">|</span> 
                                <span class="text-indigo-600 lowercase tracking-normal text-[11px] font-semibold">{{ auth()->user()->name }}</span>
                            @else 
                                <span class="text-gray-400">PREMIUM LAUNDRY SERVICE</span> 
                            @endauth
                        </span>
                    </div>
                </a>
            </div>

            {{-- Navigation Action Section --}}
            <div class="flex items-center gap-6">
                
                {{-- 1. Desktop Links (Functional Roles Only) --}}
                <div class="hidden lg:flex lg:items-center lg:gap-2">
                    @auth
                        {{-- Customer Home & Public Services inside panel --}}
                        @if(auth()->user()->hasRole('CUSTOMER'))
                            <x-nav-link :href="url('/')" :active="request()->is('/')">Home</x-nav-link>
                            <x-nav-link :href="route('services.index')" :active="request()->routeIs('services.index')">Our Services</x-nav-link>
                        @endif

                        {{-- Shared Dashboard --}}
                        <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">Dashboard</x-nav-link>

                        {{-- Role-based links --}}
                        @can('create orders')
                            <x-nav-link :href="route('orders.create')" :active="request()->routeIs('orders.create')">
                                {{ auth()->user()->hasRole('CUSTOMER') ? __('Book Service') : __('Walk-in Order') }}
                            </x-nav-link>
                        @endcan

                        {{-- Admin/Staff management links --}}
                        @can('manage staff')
                            <x-nav-link :href="route('admin.staff.index')" :active="request()->routeIs('admin.staff.*')">Staff</x-nav-link>
                        @endcan

                        @can('manage services')
                            <x-nav-link :href="route('admin.services.index')" :active="request()->routeIs('admin.services.*')">Services</x-nav-link>
                        @endcan
                    @else
                        {{-- Guest Navigation --}}
                        <x-nav-link :href="url('/')" :active="request()->is('/')">Home</x-nav-link>
                        <x-nav-link :href="route('services.index')" :active="request()->routeIs('services.index')">Services</x-nav-link>
                    @endauth
                </div>

                {{-- 2. Master Menu Trigger (For Mobile and Quick Access) --}}
                <button @click="open = ! open" 
                        class="flex items-center gap-3 p-2 pl-4 bg-slate-50 border border-gray-100 rounded-2xl hover:bg-gray-100 transition-all active:scale-95 group shadow-sm">
                    <span class="hidden md:block text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 group-hover:text-gray-900 transition-colors">
                        @auth Menu @else Access @endauth
                    </span>
                    <div class="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm border border-gray-200">
                        <svg class="h-5 w-5 text-gray-600 transition-transform duration-300" :class="{'rotate-90': open}" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                            <path x-show="!open" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                            <path x-show="open" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </div>
                </button>
            </div>
        </div>
    </div>

    {{-- Dropdown Menu --}}
    <div x-show="open" 
         x-cloak
         x-transition:enter="transition ease-out duration-200"
         x-transition:enter-start="opacity-0 translate-y-4"
         x-transition:enter-end="opacity-100 translate-y-0"
         x-transition:leave="transition ease-in duration-150"
         x-transition:leave-start="opacity-100 translate-y-0"
         x-transition:leave-end="opacity-0 translate-y-4"
         @click.away="open = false"
         class="absolute right-4 md:right-10 top-[88px] w-72 bg-white border border-gray-100 rounded-[2rem] shadow-2xl z-50 overflow-hidden">
        
        <div class="p-6">
            @auth
                {{-- User Info Header --}}
                <div class="flex items-center px-4 py-4 bg-slate-50 rounded-2xl mb-6">
                    <div class="w-10 h-10 bg-gray-900 rounded-xl flex items-center justify-center text-white text-xs font-black shadow-lg uppercase">
                        {{ substr(auth()->user()->name, 0, 1) }}
                    </div>
                    <div class="ml-3 overflow-hidden">
                        <p class="text-xs font-black text-gray-900 uppercase leading-none truncate">{{ auth()->user()->name }}</p>
                        <p class="text-[10px] text-indigo-500 font-bold uppercase tracking-widest mt-1.5">{{ auth()->user()->getRoleNames()->first() }}</p>
                    </div>
                </div>

                {{-- Quick Nav for Mobile Only (matches desktop links) --}}
                <div class="lg:hidden space-y-1 mb-6 border-b border-gray-50 pb-6">
                     <a href="{{ route('dashboard') }}" class="block px-4 py-2 text-xs font-bold text-gray-600">Dashboard</a>
                     @if(auth()->user()->hasRole('CUSTOMER'))
                        <a href="{{ url('/') }}" class="block px-4 py-2 text-xs font-bold text-gray-600">Home</a>
                        <a href="{{ route('services.index') }}" class="block px-4 py-2 text-xs font-bold text-gray-600">Our Services</a>
                     @endif
                </div>

                {{-- Account Settings --}}
                <div class="space-y-1">
                    <p class="px-4 text-[9px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-3">System Access</p>
                    <a href="{{ route('profile.edit') }}" class="flex items-center px-4 py-3 text-xs font-bold uppercase tracking-widest text-gray-600 hover:bg-slate-50 rounded-xl transition">Settings</a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full text-left flex items-center px-4 py-3 text-xs font-bold uppercase tracking-widest text-rose-500 hover:bg-rose-50 rounded-xl transition font-black">
                            Log Out
                        </button>
                    </form>
                </div>
            @else
                {{-- Guest Navigation & Auth (Mobile Dropdown Only) --}}
                <div class="space-y-3 lg:hidden">
                    <a href="{{ url('/') }}" class="block px-4 py-3 text-xs font-bold uppercase tracking-widest text-gray-600 hover:bg-slate-50 rounded-xl text-center">Home</a>
                    <a href="{{ route('services.index') }}" class="block px-4 py-3 text-xs font-bold uppercase tracking-widest text-gray-600 hover:bg-slate-50 rounded-xl text-center">Our Services</a>
                    
                    <div class="pt-4 border-t border-gray-50 space-y-3">
                        <a href="{{ route('login') }}" class="block px-4 py-4 text-xs font-black uppercase tracking-widest text-gray-700 hover:bg-slate-50 rounded-2xl text-center border border-gray-100 transition-all active:scale-95">Log In</a>
                        <a href="{{ route('register') }}" class="block px-4 py-4 text-xs font-black uppercase tracking-widest text-white bg-indigo-600 rounded-2xl text-center shadow-lg shadow-indigo-100 transition-all active:scale-95">Sign Up Now</a>
                    </div>
                </div>
                
                {{-- Desktop Dropdown View for Guests (Profile Access) --}}
                <div class="hidden lg:block space-y-3">
                    <p class="text-[9px] font-black text-center text-gray-400 uppercase tracking-widest">Authentication Required</p>
                    <a href="{{ route('login') }}" class="block px-4 py-4 text-xs font-black text-center text-indigo-600 bg-indigo-50 rounded-2xl">Log In to Account</a>
                </div>
            @endauth
        </div>
    </div>
</nav>